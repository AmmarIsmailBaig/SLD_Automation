...
def place_block(msp, spec, x, unit):
    ...
    stamp_attribs(ref, resolve_attribs(spec, unit))
...
def place_bubble(msp, spec, x, label):
    ...
    if "down_to_y" in leads:
        msp.add_line((cx, cy - radius), (cx, leads["down_to_y"]),
                     dxfattribs={"layer": "SLD_CONTROL"})
...
def bus_extents(cfg, units):
    ...
    for bus in cfg.get("control", {}).get("buses", []):
        ...
        x0 = centreline_x(cfg, indices[0]) + bus_dx_override(
            cfg, first["archetype"], name, bus.get("start_dx", 0.0))
        x1 = centreline_x(cfg, indices[-1]) + bus_dx_override(
...
def draw_control_wiring(msp, cfg, units, buses, conductors):
    ...
    for i, unit in enumerate(units):
        ...
        for riser in wiring.get("risers", []):
            ...
            # gets a junction dot; one that merely passes over got an arc.
    ...
    for bus in buses:
        draw_horizontal(msp, bus["y"], bus["x0"], bus["x1"],
...
def add_mtext(msp, text, x, style):
    ...
    m = msp.add_mtext(wrap_text(text, style.get("wrap")),
                      dxfattribs={"layer": "SLD_TEXT",
                                  "char_height": style["height"]})
    m.dxf.width = 0.0
    m.set_location((x + style.get("dx", 0.0), style["y"]),
                   attachment_point=align.TOP_CENTER if centred else align.TOP_LEFT)
    return m
...
def text_styles(cfg, archetype):
    ...
    """
    styles = {k: dict(v) for k, v in cfg["text"].items()}
...
def annotate_unit(msp, cfg, unit, x):
    ...
    spec = "\\P".join(p for p in (unit.get("tag"), unit.get("voltage"),
                                  unit.get("amp_rating"), unit.get("ka_rating")) if p)
    add_mtext(msp, spec, x, t["spec_block"])
...
def build(units, cfg, doc):
    ...
    def bus_edge(index, sign):
        arch = cfg["archetypes"].get(units[index]["archetype"], {})
        overhang = 0.0 if arch.get("bus_terminates_here") else sheet["bus_extension"]
        return centreline_x(cfg, index) + sign * overhang

    x_start = bus_edge(0, -1)
    x_end = bus_edge(len(units) - 1, +1)
    msp.add_line((x_start, bus_y), (x_end, bus_y), dxfattribs={"layer": "SLD_BUS"})

    bus_name = (units[0].get("bus") or "").strip() or "A"
    add_mtext(msp, sheet["bus_label"].replace("{bus}", bus_name),
              centreline_x(cfg, 0), cfg["text"]["bus_label"])
    ...
    for i, unit in enumerate(units):
        ...
        # Conductor from the bus down to the lowest series terminal. An
        # archetype may pull that terminal explicitly: the tie unit leaves its
        # cubicle sideways at 11.246 rather than ending on a connector.
        ...
        if y_bottom is not None:
            conductors += draw_conductor(msp, x, bus_y, y_bottom, conductor_gaps(roles))
...
def main():
    ...
    units = load_units(args.units_csv, args.bus)
    if not units:
